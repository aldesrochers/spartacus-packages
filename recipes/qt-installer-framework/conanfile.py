...
class QtInstallerFrameworkConan(ConanFile):
    ...
    license = "LGPL"
    description = "The Qt installer framework."
    ...
    def configure(self):
        ...
        # always link to qt5-static
        self.options["qt5"].shared = False

    def source(self):
        tools.get(**self.conan_data["sources"][self.version])
        os.rename("qt-installer-framework-{}".format(self.version), self._source_subfolder)
    ...
    def package(self):
        self.copy("*", src=os.path.join(self._build_subfolder, "bin"), dst="bin", keep_path=True)
